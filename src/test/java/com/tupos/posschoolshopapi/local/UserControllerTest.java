...
class UserControllerTest {
    ...
    @Test
    void adminPuedeVerUsuarios() throws Exception {
    ...
    void adminPuedeCrearUsuario() throws Exception {
        mockMvc.perform(post("/api/auth/users")
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{ \"username\": \"testuser_" + System.currentTimeMillis() + "\", \"password\": \"test12345\", \"role\": \"EMPLOYEE\" }"))
                .andExpect(status().isOk());
    }

    @Test
    void empleadoNoPuedeVerUsuarios() throws Exception {
        MvcResult result = mockMvc.perform(post("/api/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
